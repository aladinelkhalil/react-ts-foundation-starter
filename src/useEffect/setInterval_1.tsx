...
/**
 * This example demonstrates a problem that may occur when using a state
...
const Counter = (): JSX.Element => {
	 ...
	 * once: this is because the useEffect callback captures the value of count = 0
	 * when it's created, which happens the first time the component is rendered.
	 * This results in setCount(0 + 1) every time the timer updates, and since the
	 * new value to setCount is the same, React doesn't trigger a re-render.
	 *
	 ...
	 * render, the useEffect will run again to create a new timer but calls the cleanup
	 * function first to destroy the previous timer.
	 */
